...
export class RoomItemComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(){
    ...
    this.service.reservation$.subscribe(
      (data) => {
        if (data) {
  ...
  ngOnDestroy(){
   //this.service.reservation$.unsubscribe()
  }
 ...
 countTotalPrice(){
  ...
  switch (this.reservation.food){
    case 'none':
      ...
      break;
    case 'all':
      price = this.reservation.days*(this.room!.price + this.reservation.guests*40)
      break;
    case 'only':
      price = price = this.reservation.days*(this.room!.price + this.reservation.guests*15)
      break;
    default:
      price = price = this.reservation.days*(this.room!.price + this.reservation.guests*15)
  }
